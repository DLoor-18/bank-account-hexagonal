...
function getRootTAttributesFromSelector(selector) {
  const {
    attrs,
    classes
  } = extractAttrsAndClassesFromSelector(selector);
  const tAtts = attrs;
  if (classes.length) {
    tAtts.push(1, ...classes);
  }
  return tAtts;
}
  ...
  create(injector, projectableNodes, rootSelectorOrNode, environmentInjector) {
    ...
    try {
      ...
      try {
        ...
        componentView = createRootComponentView(hostTNode2, hostRNode, rootComponentDef, rootDirectives, rootLView, environment);
  ...
  return __spreadProps(__spreadValues({}, REACTIVE_NODE2), {
    ...
    producerRecomputeValue(node) {
      ...
      try {
        ...
        wasEqual = oldValue !== UNSET$1 && oldValue !== ERRORED$1 && newValue !== ERRORED$1 && node.equal(oldValue, newValue);
      } catch (err) {
        newValue = ERRORED$1;
        node.error = err;
      } finally {
...
var LINKED_SIGNAL_NODE$1 = (() => {
  return __spreadProps(__spreadValues({}, REACTIVE_NODE2), {
    ...
    producerRecomputeValue(node) {
      ...
      try {
        const newSourceValue = node.source();
        const prev = oldValue === UNSET$1 || oldValue === ERRORED$1 ? void 0 : {
          source: node.sourceValue,
          value: oldValue
        };
        newValue = node.computation(newSourceValue, prev);
...
function linkedSignal(optionsOrComputation, options) {
  ...
  const node = Object.create(LINKED_SIGNAL_NODE2);
...
@angular/core/fesm2022/core.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-AB24NWSR.js.map
